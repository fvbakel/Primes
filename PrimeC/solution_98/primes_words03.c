...
#if defined COMPILE_64_BIT
#define TYPE uint64_t
#define MASK 63U
#define SHIFT 6U
#define MEMSHIFT 7U
#define SHIFTSIZE 3U
#elif defined COMPILE_8_BIT
#define TYPE uint8_t
#define MASK 7
#define SHIFT 3
#define MEMSHIFT 4U
...
void run_sieve_segment(
    ...
    unsigned int prime;
    unsigned int max_index=(end_nr>>1U)  - !(end_nr & 1U);
...
void run_sieve(struct sieve_state *sieve_state) {
    ...
    // (sieve_state->limit >> 1) / 100;  ~= sieve_state->limit >> 8U; 
    unsigned int max_product = sieve_state->limit >> 8U; 
    ...
    if (sieve_state->limit < 100000) {
        ...
        // STEP 1:
        // First calculate all the primes in the first 2 words
        // sizeof(TYPE)<<3U == sizeof(TYPE) * 8
        run_sieve_segment(sieve_state,1,(sizeof(TYPE)*8*2),0);

        // STEP 2:
        // find the optimal product of primes for repeat pattern
        // in the first half word
        for (unsigned int num = 1; num <= (sizeof(TYPE)*8*2); num ++) {
...
void print_primes (struct sieve_state *sieve_state) {
    unsigned int max_index=(sieve_state->limit>>1U) - !(sieve_state->limit & 1U);
    printf("%i,",2);
        ...
        int             passes) 
{
	...
	printf("\n");
	printf("fvbakel_Cwords3;%d;%f;1;algorithm=other,faithful=yes,bits=%lu\n", passes, duration,1LU);
}
